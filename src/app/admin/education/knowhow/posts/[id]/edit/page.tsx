...
export default function AdminKnowHowPostEditPage() {
  ...
  const { data: post, isLoading: postLoading } = useQuery<KnowHowPost>({
    queryKey: ['knowhow-post', id],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (post) {
      ...
      setUploadedImages(post.imageUrls || []);
      setUploadedFiles(
  ...
  const updatePostMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('커뮤니티 글 수정 완료', {
        description: '커뮤니티 글이 성공적으로 수정되었습니다.',
      });
      router.push('/admin/education/knowhow');
    },
    onError: (error: Error) => {
      toast.error('커뮤니티 글 수정 실패', {
  ...
  if (!post) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="text-center text-red-600">게시글을 찾을 수 없습니다.</div>
      </div>
    );
  }
