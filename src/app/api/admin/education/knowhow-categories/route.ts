...
import {
  ...
  type CreateKnowHowCategoryInput,
} from '@/lib/validations/education';
import { successResponse, errorResponse, ErrorCode } from '@/lib/api/response';
import { ZodError } from 'zod';

// POST /api/admin/education/knowhow-categories - 노하우 카테고리 생성 (관리자 전용)
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. 유효성 검증 (Zod)
    const validatedData: CreateKnowHowCategoryInput = createKnowHowCategorySchema.parse(body);
    ...
    // 서버 에러
    console.error('KnowHow category creation error:', error);
    return errorResponse(
...
// GET /api/admin/education/knowhow-categories - 노하우 카테고리 목록 조회
