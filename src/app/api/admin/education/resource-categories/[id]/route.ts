...
// GET /api/admin/education/resource-categories/[id] - 자료실 카테고리 상세 조회
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Resource category detail error:', error);
...
// PATCH /api/admin/education/resource-categories/[id] - 자료실 카테고리 수정 (관리자 전용)
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 3. 유효성 검증 (Zod)
    const validatedData: UpdateResourceCategoryInput = updateResourceCategorySchema.parse(body);
    ...
    // 서버 에러
    console.error('Resource category update error:', error);
    return errorResponse(
      ErrorCode.INTERNAL_ERROR,
...
// DELETE /api/admin/education/resource-categories/[id] - 자료실 카테고리 삭제 (관리자 전용)
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 4. 자료가 있는 카테고리는 삭제 불가
    if (count && count > 0) {
      return errorResponse(
        ErrorCode.VALIDATION_ERROR,
        '자료가 있는 카테고리는 삭제할 수 없습니다',
        { resourceCount: count },
        400
      );
    }
    ...
    // 5. 카테고리 삭제
    const { error: deleteError } = await supabase.from('resource_categories').delete().eq('id', id);
    ...
    // 서버 에러
    console.error('Resource category delete error:', error);
    return errorResponse(
      ErrorCode.INTERNAL_ERROR,
